...
use App\Models\AgencyPpm;

class RegisterRewardCollaborate extends Component
{
    ...
    public $campaignRewardId, $reward_amount;
    public $campaign;
    ...
    public $loadindPay;
    // data pagosNet
    public $message, $bannerStyle, $ipapi, $codigoRecaudacionPn, $idTransaccionPn;
    public $host;

    public function mount(Campaign $campaign, $campaignRewardId)
    {
        if($campaign->status == 'PUBLISHED') {
            ...
            if($this->campaign->agency->country->code == $this->country_code) {
                ...
                $this->campaignReward =  CampaignReward::findOrFail($this->campaignRewardId);
                $this->amount_user = $this->campaignReward->amount;
                $this->amountTotal();
                ...
                $this->campaignReward =  CampaignReward::findOrFail($this->campaignRewardId);
                $this->amount_user = $this->convertCurrency($this->campaignReward->amount, $this->campaignReward->campaign->agency->id, $this->campaignReward->campaign->agency->agencySetting->money_id);
                $this->amountTotal();
            }
    ...
    {   
        ...
        return view('livewire.campaigns.collaborate.register-reward-collaborate');
    }
    // validate
    protected $rules = [
    ...
    public function store($payment_method) {
        ...
        $record->paymentOrderCampaign()->create([
            ...
            'campaign_reward_id' =>  $this->campaignRewardId,
