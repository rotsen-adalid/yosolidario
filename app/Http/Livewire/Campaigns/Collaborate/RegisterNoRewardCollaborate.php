...
class RegisterNoRewardCollaborate extends Component
{
    ...
    public $name, $lastname, $show_name, $email, $phone, $telephone_prefix, $country_id, $payment_method, $commentary;
    ...
    public function mount(Campaign $campaign)
    {
        if($campaign->status == 'PUBLISHED') {
            ...
            if($this->campaign->agency->country->code == $this->country_code) {
                ...
                $this->telephone_prefix = $this->campaign->agency->country->telephone_prefix;
            } else {
    ...
    public function amountTotal() {
        if($this->amount_collaborator >= 5 and $this->amount_collaborator <= 14) {
            ...
            $this->amount_total = $this->amount_collaborator + $this->amount_percentage_yosolidario;
            ...
            $this->amount_percentage_yosolidario = 12;
            $this->amount_total = $this->amount_collaborator + ($this->amount_collaborator * $this->amount_percentage_yosolidario / 100);
        } else {
            $this->collected_percentage_ys = array(
                array("value" => "7","amount_collaborator" => 0),
                array("value" => "10","amount_collaborator" => 0),
    ...
    public function percentageAmountTotal() {
        if($this->amount_collaborator) {
            if($this->amount_percentage_yosolidario != 'OTHER') {
                ...
                if($this->amount_collaborator >= 5 and $this->amount_collaborator <= 14) {
                    $this->amount_yosolidario = 1;
                }elseif($this->amount_collaborator >= 15) {
                    $this->amount_yosolidario = $this->amount_collaborator * 12 / 100;;
                }
    ...
    public function store($payment_method) {
        ...
        if(!$this->telephone_prefix) {
            $record_country = Country::find($this->country_id);
            $this->telephone_prefix = $record_country->telephone_prefix;      
        }
        ...
        $record = PaymentOrder::create([
            ...
            'email' => $this->email,
            'phone' => $this->telephone_prefix.$this->phone,
            'show_name' => $show_name,
    ...
    private function pagosNetEfectivo() {
        ...
        $paymentOrderId = $this->store('CASH');
