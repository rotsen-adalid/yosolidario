...
namespace App\Http\Livewire\Organization\Collaborate;
...
class RegisterRewardOrganizationCollaborate extends Component
{
    use InteractsWithBanner;
    use Collaborate;
    use Utilities;

    public $organization, $organizationReward, $organizationRewardId;
    ...
    public function mount(Organization $organization, $organizationRewardId)
    {
        if($organization->organizationAgreement->status == 'ACTIVE') {
            $this->organization = $organization;
            $this->organizationRewardId = $organizationRewardId;
            // ipapi
            ...
            if($this->organization->agency->country->code == $this->country_code) {
                $this->currency = $this->organization->agency->agencySetting->money->currency_symbol;
                $this->money_id = $this->organization->agency->agencySetting->money->id;
                // $this->phone_prefix = $this->organization->agency->country->phone_prefix;
            } else {
            ...
            if($this->organization->agency->country->code == $this->country_code) {
                ...
                // reward
                $this->organizationReward = OrganizationReward::find($organizationRewardId);
                $this->amount_user = $this->organizationReward->amount;
                $this->amountTotal();
            } else {
                
                // reward
                $this->organizationReward = OrganizationReward::find($organizationRewardId);
                $this->amount_user = $this->convertCurrency($this->organizationReward->amount, $this->organization->agency->id, $this->organization->agency->agencySetting->money_id);
                $this->amountTotal();
            }
    ...
    {
        $this->organizationReward =  OrganizationReward::findOrFail($this->organizationRewardId);
        return view('livewire.organization.collaborate.register-reward-organization-collaborate');
    }
// validate
    protected $rules = [
    ...
    public function store($payment_method) {
        ...
        $record = PaymentOrder::create([
            ...
            'agency_id' => $this->organization->agency->id,
        ...
        $record->paymentOrderOrganization()->create([
            'organization_id' => $this->organization->id,
            'organization_reward_id' => $this->organizationRewardId,
            'type_collaboration' => 'WITH_REWARD',
        ]);
