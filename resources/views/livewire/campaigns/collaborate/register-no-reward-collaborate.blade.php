        ...
        <div class="md:grid md:grid-cols-3 md:gap-6">
            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                ...
                <div class="p-5 sm:px-16 sm:pt-10 sm:pb-16 bg-white space-y-6 ">
                    <a href="{{ route('campaign/published',  $this->campaign->slug) }}" 
                    ...
                    <div class="flex flex-col sm:flex-row space-x-2 items-center">
                        <div>
                            <img src="{{ URL::to('/').$this->campaign->image->url}}" alt="" class="h-20 w-28 sm:h-20 sm:w-24 object-cover">
                        </div>
                        <div>
                            <div>
                                <span class="text-base">{{__('You\'re supporting')}}</span>
                                <span class="font-bold">{{$campaign->title}}</span>
                            </div>
                            <div>
                                <span class="text-gray-600">{{__('Your donation will benefit')}}</span>
                                @if ($this->campaign->type_campaign == 'PERSONAL' or $this->campaign->type_campaign == 'PERSONAL_ORGANIZATION')
                                    <span class="font-bold">{{$campaign->user->name}}</span>
                                @elseif ($this->campaign->type_campaign == 'ORGANIZATION')
                                    <span class="font-bold">{{$campaign->organization->name}}</span>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="mb-4">
                            @if ($this->campaign->categoryCampaign->type == 'SOCIAL_IMPACT')
                                <span class="font-bold uppercase">{{__('Enter your donation')}}</span>
                            @elseif ($this->campaign->categoryCampaign->type == 'PROJECT')
                                <span class="font-bold uppercase">{{__('Enter your contribution')}}</span>
                            @endif
                        ...
                        <div class="flex justify-center">
                            <div class="flex rounded-md shadow-sm sm:mx-0">
                                <span class="inline-flex items-center px-3 rounded-l-md border border-r-0 border-gray-300 bg-white text-gray-900 text-2xl sm:text-4xl font-bold">
                                    {{$this->currency}}
                                </span>
                                <input type="text" name="amount_user" id="amount_user" wire:model="amount_user"  wire:keyup="amountTotal" onKeyPress="return validar(event)" maxlength="9" autocomplete="off"
                                class="focus:ring focus:ring-gray-50 focus:ring-opacity-50 focus:border-gray-200 flex-1 block w-full rounded-none text-2xl sm:text-4xl border-l-0 border-r-0 border-gray-200 font-bold shadow-xs text-right  bg-white" placeholder="">
                                <span class="inline-flex items-center pr-3 rounded-r-md border border-l-0 border-gray-300 bg-white text-gray-900 text-2xl sm:text-4xl font-bold  bg-white">
                                    .00
                                </span>
                            </div>
                        </div>
                        
                        <x-input-error for="amount_user" class="mt-2" />
                    </div>
                        ...
                        <div class="flex space-x-5 mt-4">
                            <div class="w-6/12">
                                <x-select class="mt-1 w-full" id="amount_percentage_yosolidario" wire:model="amount_percentage_yosolidario" wire:change="percentageAmountTotal">
                                    <x-slot name="option">
                                        @foreach ($collected_percentage_ys as $this->campaignReward)
                                            <option value="{{$this->campaignReward['value']}}">
                                                <!-- 
                                                if ($this->amount_user >= 5 and $this->amount_user <= 14)
                                                    <span>{$this->campaignReward['amount_user']}}</span>
                                                    <span>
                                                        {$this->currency}}
                                                    </span>
                                                -->
                                                @if($this->amount_user >= 5)
                                                    <span>{{$this->campaignReward['value']}}%</span>
                                                    @if ($this->campaignReward['amount_user'] != 0)
                                                        <span>
                                                            (<span>{{ number_format($this->campaignReward['amount_user'], 2 ) }}</span>
                                                            <span>
                                                                {{$this->currency}}
                                                            </span>)
                                                        </span>
                            ...
                            <div class="w-full">
                                ...
                                <x-select class="mt-1 block w-full" id="country_id" name="country_id" wire:model="country_id">
                                    <x-slot name="option">
                                        ...
                                        foreach ($collection_countries as $this->campaignReward)
                                            <option value="{$this->campaignReward->id}}">{ __($this->campaignReward->name) }}</option>
                                        endforeach
                                    </x-slot>
                                </x-select>
                            </div>
                            <div class="w-full">
                                ...
                                <x-select class="mt-1 block w-full" id="country_state_id" name="country_state_id" wire:model="country_state_id">
                                    <x-slot name="option">
                                        ...
                                        foreach ($collection_country_states as $this->campaignReward)
                                            <option value="{$this->campaignReward->id}}">{__($this->campaignReward->name) }}</option>
                                        endforeach
                                    </x-slot>
                                </x-select>
            ...
            <!-- -->
            <div class="pt-10 pb-5 border-b border-gray-200">
                <div class="uppercase font-bold ">{{$campaign->title}}</div>
                ...
                <div class="flex mt-5">
                    ...
                    <div class="ml-3 space-y-0">
                        <div wire:click="viewUser({{$this->campaign->user->id}})" class="text-sm sm:text-base cursor-pointer text-semibold"> 
                            {{$this->campaign->user->name}} {{$this->campaign->user->lastname}}
                        </div>
                ...
                <!-- ORGANIZATION -->
                <div class="flex mt-5">
                    <div wire:click="viewUser({{$this->campaign->organization->id}})" wire:loading.attr="disabled" class="flex-shrink-0 w-12 h-12 cursor-pointer">
                        <img class="w-full h-full rounded-full object-cover"
