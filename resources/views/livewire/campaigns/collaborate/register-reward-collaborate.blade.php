        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-2 py-10">
            <div class="md:grid md:grid-cols-3 md:gap-6">
                        ...
                        <div>
                            ...
                            <div class="flex justify-center ">
                                <div class="flex rounded-md shadow-sm sm:mx-0 w-full bg-gray-100">
                                    @if ($campaignReward->campaign->agency->country->code == $this->country_code)
                                        ...
                                        <span class="inline-flex py-2 items-center pr-3 rounded-r-md border border-l-0 border-gray-300 text-gray-900 text-2xl sm:text-4xl font-bold ">
                                            {{ number_format($campaignReward->amount, 2 )}}
                                        </span>
                                    @else
                                        <span class="w-full py-2 inline-flex items-center px-3 rounded-l-md border border-r-0 border-gray-300 text-gray-900 text-2xl sm:text-4xl font-bold">
                                            {{$this->currency}}
                                        ...
                                        <span class="inline-flex py-2 items-center pr-3 rounded-r-md border border-l-0 border-gray-300 text-gray-900 text-2xl sm:text-4xl font-bold ">
                                            {{  number_format(
                                                $this->convertCurrency(
                                                    $campaignReward->amount, 
                                                    $campaignReward->campaign->agency->id,
                                                    $campaignReward->campaign->agency->agencySetting->money_id
                                                ), 2 ) }}
                                        </span>
                                    @endif
                                </span>
                ...
                <div class=" mt-6  max-w-sm mx-auto">
                    ...
                    @if($campaignReward->image_url)
                        <div class="cursor-pointer bg-gray-300 h-56 w-full rounded-lg bg-cover bg-center" 
                            style="background-image: url({{ URL::to('/').$campaignReward->image_url}})">
                        </div>
                    @endif
                    <div class=" w-full bg-white @if($campaignReward->image_url) -mt-10 @endif  shadow-md rounded-lg overflow-hidden p-5">
                        @if ($campaignReward->campaign->agency->country->code == $this->country_code)
                        ...
                        <div class="summary-post text-base text-justify mt-4">
                            {{$campaignReward->description}}
                        </div>
                        <!-- delivery_date -->
                        @if ($campaignReward->delivery_date)
                        <div class="text-sm text-justify mt-1">
                            {{__('Estimated delivery date')}}
                            <span class="font-semibold">
                                {{ \Carbon\Carbon::parse($campaignReward->delivery_date)->toFormattedDateString() }}
                            </span>
                        </div>
                        @endif
                    </div>
