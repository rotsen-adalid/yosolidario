<div class=" py-0 sm:px-0">
    @if ($sumCollection->count() > 0)
        ...
        <div class="truncate-single-line flex justify-center sm:justify-end items-center pb-6">
            <div wire:click="maxCollaborations" wire:loading.attr="disabled"
                class="border border-gray-100 py-1 px-2 text-gray-800 shadow-md rounded-full cursor-pointer flex items-center space-x-1">
                <span class="material-icons-outlined text-sm">share</span>
                <span class="text-sm sm:text-sm">{{__('Ver colaboraciones mas importantes')}}</span>
            </div>
        </div>
        ...
        <div class="flex justify-between">
            <div class="flex items-center ">
                ...
                <div class="ml-3 space-y-2">
                    <div class="text-gray-700 text-sm sm:text-base cursor-pointer"> 
                        ...
                        <div class="truncate-single-line flex space-x-2 mt-1">
                            <div class="font-bold">
                                {{ number_format($item->paymentOrder->amount_user, 0 ) }}
                                {{$item->paymentOrder->money->currency_symbol}}
                            </div>
                            <div class="font-bold flex items-center text-gray-500">
            ...
            <div class="truncate-single-line">
                @if ($item->paymentOrder->paymentOrderCampaign)
                    @if ($item->paymentOrder->paymentOrderCampaign->campaignReward)
                        <div class="border border-purple-200 bg-purple-100 text-purple-800 py-0 px-2 rounded-full text-xs sm:text-sm transition ease-in-out duration-150">
                            {{__('Reward')}}
                        </div>
                    @else 
                        <div class="border border-yellow-200 bg-yellow-100 text-yellow-800 py-0 px-2 rounded-full text-xs sm:text-sm transition ease-in-out duration-150">
                            {{__('Donation')}}
                        </div>
                    @endif
                @endif
            ...
            @if ($dn == 4)
                ...
                <div class="border-t border-b border-gray-200 px-4 my-3 flex space-x-2 items-center h-12" style="background-color:#fbf8f6">
            ...
            <div>
                ...
                <div class=" flex sm:justify-center sm:items-center mt-5">
                    <button
                        ...
                        class="w-full lg:w-72 px-4 py-2 sm:py-3 text-center bg-yellow-400 border border-yellow-500 shadow-lg rounded-md font-bold text-base text-black uppercase tracking-widest hover:bg-yellow-300 active:bg-yellow-500 focus:outline-none focus:border-gray-100 focus:shadow-outline-gray disabled:opacity-25 transition ease-in-out duration-150">
